...
///
// VERIFIED: privacy-md/sensitive-clipboard
// The watcher's worker_step calls is_sensitive() FIRST, before any
// persistence. macOS impl honors NSPasteboard concealed types
// (org.nspasteboard.ConcealedType, AutoGeneratedType, TransientType).
// Windows impl honors ExcludeClipboardContentFromMonitoring +
// CanIncludeInClipboardHistory=0 + CanUploadToCloudClipboard=0.
// When any flag is set, the event is skipped with
// SkipReason::SensitiveFlag and NEVER reaches the local DB.
#[derive(Default)]
pub struct OsProbe;
