...
# Windows-only manifest embedding via embed-resource.
...
If the `embed-resource` crate API differs (versions vary), the simpler fallback is `embed_resource::compile("smoke-target.exe.manifest", embed_resource::NONE);`. Check `cargo doc --open -p embed-resource` once if the build fails.

**Step 4: Create the Windows manifest**
...
If the build fails on `embed-resource` API, swap `compile_for_everything` for `compile` and rebuild.

**Step 7: Verify the produced binary runs**
