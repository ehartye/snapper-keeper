...
- Modify: `crates/snk-ocr/build.rs` — append `"get_ocr_words"` to the `COMMANDS` array (plan amendment 2026-05-26 — winocr-pc post-T10 catch. Tauri's `tauri_plugin::Builder::new(COMMANDS).build()` autogenerates the per-command permission files under `crates/snk-ocr/permissions/autogenerated/commands/`, which the `snk-ocr:default` capability then includes. Without the array update, calling `invoke('plugin:snk-ocr|get_ocr_words')` from TS fails with a capability denial at runtime. Convention: every entry in `invoke_handler!` MUST appear in `COMMANDS`. Cross-plugin check: `snk-annotate`, `snk-clipboard`, `snk-popup` all enforce this.)
